#!/usr/bin/env python3
"""Under the Bar - Plot Cumulative Reps
...
def generate_options_cumulative_reps():
	...
	for user_file in user_files:
		...
		for set_group in workout_data['exercises']:
			if set_group["exercise_type"] == "weight_reps" or set_group["exercise_type"] == "reps_only" or set_group["exercise_type"] == "bodyweight_reps":
				...
				filename = user_folder+'/plot_cumulativereps_'+re.sub(r'\W+', '', set_group["title"])+'.svg'
...
def generate_plot_cumulative_reps(the_exercise, width, height):
	home_folder = str(Path.home())
	utb_folder = home_folder + "/.underthebar"
	session_data = {}
	if os.path.exists(utb_folder+"/session.json"):	
	...
	plt.title(the_exercise+' Cumulative Reps')
	...
	fig1.savefig(export_folder+'/plot_cumulativereps_'+re.sub(r'\W+', '', the_exercise)+'.svg', dpi=100)
