#!/usr/bin/env python3
"""Under the Bar - Plot Volume per Workout

This file provides a plot of volume per workout, can be optionally limited to the last year.
"""
...
def generate_options_volume_workout(timelimit=False):
	...
	filename = user_folder+'/plot_volumeworkout_all.svg'
	if timelimit:
		filename = user_folder+'/plot_volumeworkoutyear_all.svg'
	...
	for user_file in user_files:
		...
		for set_group in workout_data['exercises']:
			if set_group["exercise_type"] == "weight_reps" or set_group["title"] in special_bodyweight:
				...
				filename = user_folder+'/plot_volumeworkout_'+re.sub(r'\W+', '', set_group["title"])+'.svg'
				if timelimit:
					filename = user_folder+'/plot_volumeworkoutyear_'+re.sub(r'\W+', '', set_group["title"])+'.svg'
				exists = os.path.exists(filename)
...
def generate_plot_volume_workout(the_exercise, width, height, timelimit=False):
	home_folder = str(Path.home())
	utb_folder = home_folder + "/.underthebar"
	session_data = {}
	if os.path.exists(utb_folder+"/session.json"):	
	...
	plt.title(the_exercise+' Volume (per Workout)')
	...
	if timelimit:
		fig1.savefig(export_folder+'/plot_volumeworkoutyear_'+re.sub(r'\W+', '', the_exercise)+'.svg', dpi=100)
	else:
		fig1.savefig(export_folder+'/plot_volumeworkout_'+re.sub(r'\W+', '', the_exercise)+'.svg', dpi=100)
